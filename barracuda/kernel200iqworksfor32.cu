...
__global__ void GPUrest(const int* const __restrict__ contacts, const int* const __restrict__ in, int* const infections, const int n, const int iter, int* const out, int* const rest){
    ...
    if(blockIdx.y % 4 == 0 || (blockIdx.y + 1) % 4 == 0 && x < n && y < (n / BLOCK) * 4){
        if(threadIdx.x == 0){
                ...
                neighborhood[2][0] = x != 0 && y < (n / BLOCK) * 4 - 1 ? rest[(y+1)*n + (x-1)] : 0;
            }
            ...
            if(threadIdx.x == maxIdx-1){
                if(maxIdx == BLOCK){
                    ...
                    neighborhood[2][maxIdx + 1] = blockIdx.y < (n / BLOCK) * 4 - 1 && blockIdx.x < ceil((float)n/BLOCK) - 1 ? rest[(y+1)*n + (x+1)] : 0;
                }
            ...
            neighborhood[0][tid + 1] = blockIdx.y != 0 ? rest[(y-1)*n + x] : 0;
            neighborhood[1][tid + 1] = rest[pos]; 
            neighborhood[2][tid + 1] = blockIdx.y < (n / BLOCK) * 4 - 1 ? rest[(y+1)*n + x] : 0;
    }
    ...
    if(x < n && y < (n / BLOCK) * 4){
            ...
            if (in_pos == 0) {
                ...
                int lNum;
                if(blockIdx.y % 4 == 0){
                    lNum = x * n + ((blockIdx.y >> 2) * BLOCK); 
                }
                if((blockIdx.y + 1) % 4 == 0){
                    lNum = x * n + ((blockIdx.y + 1 >> 2) * BLOCK) - 1; 
                }

                if (infected > contacts[lNum]) {
                    res[tid] = 10;
...
__global__ void GPUiter( const int* const __restrict__ contacts, const int* const in, int* const infections, const int n, const int iter, int* const out, int* const rest){
        ...
        int tid = threadIdx.x;
        int x = (blockIdx.x * blockDim.x) + tid;
        int y = (blockIdx.y * LANES) + threadIdx.y;
        int maxIdx = min(BLOCK, n-(blockIdx.x * blockDim.x));
        int pos = y*n + x;
        ...
        if(x < n && y < n){
            ...
            if(threadIdx.x < 3){ // Set right and left column edge values
                neighborhood[threadIdx.x][0] = 0;
                neighborhood[threadIdx.x][maxIdx+1] = 0;
            }
        }

        for (int i = 0; i < LANES + 1; ++i)
        {
            neighborhood[2][tid+1] = y + i < n - 1 ? in[(y+1+i)*n + tid] : 0;

            __syncthreads();

            if(threadIdx.x == 0){
                restTemp[0][i] = neighborhood[1][1];
                restTemp[1][i] = neighborhood[1][2];
            }
            if(threadIdx.x == BLOCK - 2){
                restTemp[2][i] = neighborhood[1][threadIdx.x+1];
                restTemp[3][i] = neighborhood[1][threadIdx.x+2];
            }

            if(x < n && y + i < n && threadIdx.x > 0 && threadIdx.x < BLOCK - 1){
                int in_pos = neighborhood[1][tid + 1];
                if (in_pos > 0) {
                    res[tid] = in_pos - 1 == 0 ? -30 : in_pos - 1;
                }
                ...
                if (in_pos == 0) {
                    ...
                    if (infected > contacts[(y+i)*n + tid]) {
                        res[tid] = 10;
                        atomicAdd(&infections[iter], 1);
                    }
                }
