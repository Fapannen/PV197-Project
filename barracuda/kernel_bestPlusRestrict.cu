...
__global__ void GPUiter( const int* const __restrict__ contacts, const int* const __restrict__ in, int* const infections, const int n, const int iter, int* const out){
        ...
        if(x < n && y < n){
            ...
            if(threadIdx.x == maxIdx-1){
                if(maxIdx == BLOCK){
                    neighborhood[0][maxIdxInc] = blockIdx.x < ceil((float)n/BLOCK) - 1 && blockIdx.y != 0 ? in[(y-1) * n + (x + 1)] : 0;
                    neighborhood[1][maxIdxInc] = blockIdx.x < ceil((float)n/BLOCK) - 1 ? in[y * n + (x+1)] : 0;
                    neighborhood[2][maxIdxInc] = blockIdx.y < n - 1 && blockIdx.x < ceil((float)n/BLOCK) - 1 ? in[(y+1)*n + (x+1)] : 0;
                }
